...
export default function LoginView() {
  ...
  const submitLogin = () => {
    ...
    //read the form
    if (isEmpty(username)) setAlerts([{ message: t("login.usernameRequired"), type: "error" }]);
    else if (isEmpty(password)) setAlerts([{ message: t("login.passwordRequired"), type: "error" }]);
    else {
  ...
  return (
    <div className="login-wrapper">
      ...
      <div className="mb-3">
        ...
        <input
          type="text"
          className="block w-full rounded-md border border-gray-300 px-3 py-1.5 text-base focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
          id="username"
          name="username"
          value={username}
