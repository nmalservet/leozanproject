 ...
 */
 function SelectList({label,values,selected,handleSelection,withFirstItem,emptyAllowed,inline}) {
    ...
    <div className={inline?'flex':''}>
            {label&&label.length>0&&<label className={'block mb-1 text-sm font-medium text-gray-900 dark:text-white'+inline?'mr-5':''}>{label}</label>}
                {(values&&values.size>0)&&
                    <select  className=" bg-gray-50 p-1 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
                    onChange={(e) => { handle(e.target.value);}} style={{width:'100%'}} defaultValue={selected}>  
